...
export default function decorate(block) {
  ...
  const label = fieldText(fields, ['ctaLabel', 'buttonText']);
  const embedUrl = getVideoEmbedUrl(fieldText(fields, ['videoUrl', 'url']));
  const image = fieldImage(fields, ['image', 'poster']);
  ...
  if (eyebrow) copy.append(makeEl('p', '', eyebrow));
  if (heading) copy.append(makeEl('h2', '', heading));
  if (label && embedUrl) {
